...
class LanguageGameActivity : BaseActivity() {
    private lateinit var binding: ActivityGameBinding
    private val gameManager by lazy { LanguageGameManager(this) }
    ...
    private fun onTimeUp() {
        ...
        tone.startTone(ToneGenerator.TONE_CDMA_ABBR_ALERT, 200)
        binding.root.postDelayed({ goNext() }, 700)
    }

    private fun goNext() {
        if (gameManager.moveNext()) {
            showQuestion()
        } else {
            goToResults()
        }
    ...
    private fun showWrongAnswerFeedback(selectedIndex: Int) {
        ...
        buttons.getOrNull(selectedIndex)?.backgroundTintList = ColorStateList.valueOf(getColor(R.color.answer_wrong))
        buttons.getOrNull(currentCorrectIndex)?.backgroundTintList = ColorStateList.valueOf(getColor(R.color.answer_correct))
    }

    private fun showCorrectAnswerFeedback(selectedIndex: Int) {
        val buttons = listOf(binding.answer1, binding.answer2, binding.answer3, binding.answer4)
        buttons.getOrNull(selectedIndex)?.backgroundTintList = ColorStateList.valueOf(getColor(R.color.answer_correct))
    }
    ...
    private fun goToResults() {
        val intent = Intent(this, ResultActivity::class.java).apply {
            ...
            putExtra(ResultActivity.EXTRA_GAME_MODE, ScoreManager.MODE_LANGUAGE)
        }
