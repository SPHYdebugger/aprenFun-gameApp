...
class SumasTorresActivityLegacy : BaseActivity() {
    ...
    private fun renderOperation(operation: TowerOperation) {
        ...
        if (operation.type == OperationType.ADD) {
            ...
            addOperatorSpacer(binding.topRow)
            addAlignedSubtractionRow(binding.topRow, topString, operation.columns)

            addOperatorCell(binding.bottomRow, "-")
            addAlignedSubtractionRow(binding.bottomRow, bottomString, operation.columns)
        }
    ...
    private fun addAlignedSubtractionRow(row: LinearLayout, number: String, columns: Int) {
        val leading = (columns - number.length).coerceAtLeast(0)
        repeat(leading) {
            row.addView(createBorrowPlaceholder())
            row.addView(createDigitCell(""))
        }
        number.forEach { digit ->
            row.addView(createCarryInput())
            row.addView(createDigitCell(digit.toString()))
        }
    }
    ...
    private fun createBorrowPlaceholder(): TextView {
        return TextView(this).apply {
    ...
    private fun checkResult() {
        if (resultInputs.any { it.text?.toString().isNullOrBlank() }) {
            Toast.makeText(this, getString(R.string.sumas_torres_fill_all), Toast.LENGTH_SHORT).show()
            return
        }

        val entered = resultInputs.joinToString(separator = "") { it.text?.toString()?.trim().orEmpty() }
        if (entered == currentOperation.result.toString()) {
            onCorrectAnswer()
        } else {
            onWrongAnswer()
        }
    ...
    private fun showCorrectResult() {
        ...
        resultInputs.forEachIndexed { index, editText ->
            editText.setText(resultText.getOrNull(index)?.toString().orEmpty())
        }
    }
